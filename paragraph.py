...
def spell_check_with_probabilities(sentence, tokenizer, model, device):
    ...
    final_sentence = reconstruct_sentence(sentence, model_output, stop_words_index, confusion_words_index,
                                          post_positions)
...
def process_paragraph(paragraph, tokenizer, model, device):
    ...
    for sentence in sentences:
        corrected_sentence, word_probabilities, corrections = spell_check_with_probabilities(
            sentence[:-1], tokenizer, model, device
        )
    ...
    if st.button("Check Spelling in Paragraph"):
        if paragraph.strip():
            corrected_paragraph, word_probabilities = process_paragraph(
                paragraph, tokenizer, model, device
            )
